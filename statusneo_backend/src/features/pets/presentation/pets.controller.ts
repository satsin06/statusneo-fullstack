...
export class PetsController {
  constructor(
    ...
    private readonly getPetRecommendationUseCase: GetPetRecommendationUseCase
  ) {}
  ...
  getRecommendation = async (req: Request, res: Response) => {
    const recommendation = await this.getPetRecommendationUseCase.execute(req.params.id as string);
